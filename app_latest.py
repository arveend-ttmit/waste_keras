...
GRASP_RATIO = 1.6      #1.3             # lower = stricter "must be gripping something"
...
OBJECT_FILL_MIN = 0.35    # centre of crop must be >35% non-skin to count as "holding something"
...
def palm_has_object(crop):
    """
    Rough check: is there something in the crop that isn't skin or background?
    Returns the fraction of non-skin pixels in the centre region.
    """
    h, w = crop.shape[:2]
    ...
    non_skin = 1.0 - (np.count_nonzero(skin) / skin.size)
    return non_skin
...
def generate_frames():
    ...
    try:
        ...
        while True:
            ...
            if frame_no % 15 == 0:
                print(f"tracking={last_box is not None}  grasping={last_grasping}  "
                      f"stable={stable_count}  locked={locked_label}  "
                      f"history={list(history)[-3:]}")
            ...
            if not tracking:
                ...
                if locked_label is not None:
                    ...
                    history.append(NOTHING_LABEL)   # nothing but skin/background in the crop
                ...
                elif frame_no % 15 == 0 and print(f"  fill={palm_has_object(frame[y1:y2, x1:x2]):.2f}"):
                    pass
